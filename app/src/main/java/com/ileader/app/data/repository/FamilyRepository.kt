...
class FamilyRepository {
    ...
    suspend fun getFamilyLinks(userId: String): List<FamilyLinkDto> =
        MemoryCache.cached("family:links:$userId", ttlMs = 600_000L) {
            try {
    ...
    suspend fun createFamilyLink(parentId: String, childId: String) {
        ...
        MemoryCache.invalidate("family:links:$parentId")
        MemoryCache.invalidate("family:links:$childId")
    }
    ...
    suspend fun confirmFamilyLink(linkId: String) {
        ...
        // Affected parent/child unknown from id alone — invalidate all.
        MemoryCache.invalidateMatching("family:links:")
    }
    ...
    suspend fun getPendingApprovals(parentId: String): List<ParentalApprovalDto> =
        MemoryCache.cached("family:approvals:$parentId", ttlMs = 120_000L) {
            try {
                ...
                if (linkIds.isEmpty()) emptyList()
                else client.from("parental_approvals")
    ...
    suspend fun respondToApproval(approvalId: String, approved: Boolean, comment: String? = null) {
        ...
        MemoryCache.invalidateMatching("family:approvals:")
    }
}
